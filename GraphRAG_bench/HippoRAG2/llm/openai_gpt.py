...
class CacheOpenAI(BaseLLM):
    ...
    def __init__(self, cache_dir, cache_filename: str = None,
                 llm_name: str = "gpt-4o-mini", api_key: str = '8409b8cc59224a4d83632f62c26f1606',
        ...
        if high_throughput:
            ...
            client = None
        if 'azure' in self.llm_base_url.lower():
            self.openai_client = AzureOpenAI(  # base_url=self.llm_base_url,
                api_key=api_key,
                http_client=client,
    ...
    def infer(
        ...
        if 'gpt' not in params['model'] or version.parse(openai.__version__) < version.parse(
                "1.45.0"):  # if we use vllm to call openai api or if we use openai but the version is too old to use 'max_completion_tokens' argument
        ...
        try:
            ...
            response_message = 'No response due to API error.'

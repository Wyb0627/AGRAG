...
class BaseConfig:
    ...
    graph_summary: bool = field(
        default=False,
        metadata={"help": "If set to True, apply llm to summarize the graph strings."}
    )
    ...
    rag_mode: str = field(
        default="hipporag2",
        metadata={"help": "RAG mode to use. Currently, only 'hipporag2' and 'gorag' is supported."}
    )
    ner_threshold: float = field(
        default=0.1,
        metadata={"help": "Threshold for entity extraction confidence."}
    )
    max_ngram_length: int = field(
        default=3,
        metadata={"help": "Max ngram length for entity extraction."}
    )
    ...
    retrieval_mode: str = field(
        default='dense',
        metadata={"help": "Retrieval mode to use. Currently, only 'dense' and 'hybrid' is supported."}
    )
    ...
    include_passage_nodes_in_qa_input: bool = field(
        default=False,
        metadata={"help": "Whether to append passage nodes from Steiner tree to QA LLM input."}
    )
